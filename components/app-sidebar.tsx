    ...
    >
      <SidebarHeader className="h-20 border-b border-slate-100 px-6 flex flex-row items-center gap-3 bg-white">
        ...
        <div className="flex flex-col overflow-hidden group-data-[collapsible=icon]:hidden">
          ...
          <span className="text-[9px] text-indigo-600 font-extrabold tracking-widest mt-1.5 uppercase">
            Администратор
          </span>
        </div>
      </SidebarHeader>
                ...
                return (
                  <SidebarMenuItem key={item.title}>
                    ...
                    >
                      ...
                      >
                        <item.icon
                          className={`h-4 w-4 shrink-0 transition-colors ${isActive ? "text-indigo-600" : "text-slate-400"}`}
                        />
                        <span className="group-data-[collapsible=icon]:hidden text-xs font-semibold tracking-wide">
                          {item.title}
                        </span>
                      </Link>
                    </SidebarMenuButton>
