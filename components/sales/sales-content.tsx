...
export function SalesPageContent() {
  ...
  const onCreateSubmit = async (values: any, imageFiles: FileList | null) => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.error || "Произошла ошибка при добавлении скидки.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const filteredSales = sales.filter((sale) => {
    ...
    const matchesCategory = selectedCategoryId
      ? sale.categoryId === selectedCategoryId
      : true;
    const matchesSeller = selectedSellerId
      ? sale.sellerId === selectedSellerId
      : true;
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col gap-4 border-b border-slate-100 pb-6 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-slate-500 text-xs uppercase tracking-wide font-medium">
            Контроль и добавление скидочных акций и товаров на платформе.
          </p>
        </div>
        <Button
      ...
      {loading ? (
        ...
        <div className="p-12 text-center flex flex-col items-center justify-center min-h-[300px] border border-slate-100 bg-white rounded-2xl">
          ...
          <p className="mt-2 text-xs text-slate-450 max-w-sm uppercase">
            По данному запросу или в системе пока нет ни одной акции.
          </p>
        </div>
      ) : (
        <SalesTable
      ...
      <div className="flex items-start gap-3 border border-slate-100 bg-white p-4 text-[11px] text-slate-500 leading-relaxed rounded-2xl shadow-sm shadow-slate-100/20">
        ...
        <span>
          База данных скидок управляет акциями на товары в системе. Каждая акция
          должна быть обязательно привязана к соответствующему магазину продавца
          и категории. При добавлении нового товара загрузка хотя бы одного
          изображения является обязательной.
        </span>
      </div>
