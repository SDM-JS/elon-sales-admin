...
export default function UsersPage() {
  ...
  const onCreateSubmit = async (values: any) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.error || "Фойдаланувчи қўшишда хатолик юз берди.")
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const onEditSubmit = async (values: any) => {
    ...
    console.log("Update User Submit Payload:", { id: selectedUser.id, ...values })
    try {
      setSubmitting(true)
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col gap-4 border-b border-slate-100 pb-6 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-slate-500 text-xs uppercase tracking-wide font-medium">
            Тизимдан рўйхатдан ўтган харидорларни назорат қилиш ва янги аъзолар қўшиш.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="p-12 text-center flex flex-col items-center justify-center min-h-[300px] border border-slate-100 bg-white rounded-2xl">
          ...
          <h3 className="text-sm font-bold text-slate-900 uppercase tracking-widest">Аъзолар топилмади</h3>
      ...
      <div className="flex items-start gap-3 border border-slate-100 bg-white p-4 text-[11px] text-slate-500 leading-relaxed rounded-2xl shadow-sm shadow-slate-100/20">
        ...
        <span>Фойдаланувчилар базаси тизим харидорларини бошқаради. Бу ерда аъзолар исми-шарифи, телефон рақами ва махфий сўзларини таҳрирлаш ёки янги аъзолар қўшиш мумкин.</span>
      </div>
